    ...
    <div>
        ...
        <div class="container">
            <div class="handle-box">
                ...
                >批量删除</el-button>-->
                <el-input v-model="query.code" placeholder="Code码名称" class="handle-input mr10"></el-input>
            ...
            >
                ...
                <el-table-column label="Code码名称" align="center">
                    <template slot-scope="scope">{{scope.row.code.code_name}}</template>
                </el-table-column>
                <el-table-column label="兑换者昵称" align="center">
                    <template slot-scope="scope">{{scope.row.user.name}}</template>
                </el-table-column>
                <el-table-column label="兑换者open-id" align="center">
                    <template slot-scope="scope">{{scope.row.user.open_id}}</template>
                </el-table-column>
                <el-table-column prop="date" label="兑换时间" align="center">
...
<script>
import { getAllCodeLog, deleteCodeLog } from '../../api/codeLog';
export default {
    name: 'CodeLogManagement',
    data() {
        return {
            query: {
                code: '',
    ...
    methods: {
        getData() {
            getAllCodeLog({ code_log: this.query.code, page: this.query.pageIndex }).then(res => {
        ...
        handleDelete(index, row) {
            ...
            this.$confirm('确定要删除吗？', '提示', {
                ...
                .then(() => {
                    deleteCodeLog({ id: row.id }).then(res => {
                        if (res.code == 200) {
